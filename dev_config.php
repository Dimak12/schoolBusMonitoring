...
if (isset($_POST['dev_add'])) {
    ...
    elseif (empty($serv_area)) {
        ...
        $sql = "INSERT INTO bus (bus_id, serving_area) VALUES(?, ?)";
        $result = mysqli_stmt_init($conn);
        if ( !mysqli_stmt_prepare($result, $sql)){
            echo '<p class="alert alert-danger">SQL Error</p>';
        }
        else{
            mysqli_stmt_bind_param($result, "is", $bus_id, $serv_area);
            mysqli_stmt_execute($result);
            echo 1;
            $sql = "INSERT INTO bus_location (bus_id) VALUES(?)";
            $result = mysqli_stmt_init($conn);
            if ( !mysqli_stmt_prepare($result, $sql)){
                echo '<p class="alert alert-danger">SQL Error</p>';
            }
            else{
                mysqli_stmt_bind_param($result, "i", $bus_id);
                mysqli_stmt_execute($result);
                echo 1;
            }
        }
